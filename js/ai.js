...
async function aiTurn() {
    ...
    const enemyUnits = window.gameState.units
    ...
    for (const unit of enemyUnits) {
        ...
        window.gameState.aiActiveUnit = unit;
        if (window.renderAll) await window.renderAll([]);
    ...
    if (window.cleanupUnits) window.cleanupUnits();  // Clean after AI
    
    // Check victory after AI turn
    if (window.checkVictory) window.checkVictory();
    
    // Only end turn if game is still going
    const enemies = window.gameState.units.filter(u => u.type === 'enemy' && !u.fleeing);
    const players = window.gameState.units.filter(u => u.type === 'player' && !u.fleeing);
    
    if (enemies.length > 0 && players.length > 0) {
        if (window.endTurn) setTimeout(window.endTurn, 1000);
    }
}
...
async function performAIActions(unit) {
    ...
    if (unit.classType === 'mage') {
        ...
        const players = window.gameState.units.filter(u => u.type === 'player' && u.hp > 0 && !u.fleeing);
        const allies = window.gameState.units.filter(u => 
            u.type === 'enemy' && 
            u.id !== unit.id && 
            u.hp > 0 && 
            !u.fleeing
        ...
        if (unit.canHeal && unit.remainingActions > 0) {
            ...
            if (injuredAllies.length > 0) {
                ...
                if (distance <= unit.range) {
                    ...
                    if (window.performHeal) await window.performHeal(unit, target);
                    unit.remainingActions = 0;
                    return;
                }
                    ...
                    if (neededDistance <= movesLeft && neededDistance > 0) {
                        ...
                        if (path && path.length > 1) {
                            for (let i = 1; i < Math.min(path.length, movesLeft + 1); i++) {
                                ...
                                const step = path[i];
                                if (window.moveUnit) await window.moveUnit(unit, step.x, step.y);
                            }
        ...
        if (unit.canMove && unit.remainingActions > 0) {
            ...
            if (!isSafe && nearestAlly) {
                ...
                if (window.logMessage) window.logMessage(`${unit.name} seeks protection with allies.`, 'enemy');
                if (window.moveTowardTarget) await window.moveTowardTarget(unit, nearestAlly.x, nearestAlly.y, false);
                unit.remainingActions = 0;
                return;
            }
            ...
            if (!isWithPack && nearestAlly) {
                ...
                if (window.logMessage) window.logMessage(`${unit.name} regroups with the pack.`, 'enemy');
                if (window.moveTowardTarget) await window.moveTowardTarget(unit, nearestAlly.x, nearestAlly.y, false);
                unit.remainingActions = 0;
                return;
            }
            ...
            if (isSafe && isWithPack && allies.length > 0) {
                ...
                if (count > 0) {
                    ...
                    if (currentDist > 2) { // If not already near center
                        ...
                        if (window.moveTowardTarget) await window.moveTowardTarget(unit, avgX, avgY, false);
                    }
                }
            }
        }
    ...
    if (unit.classType === 'berserker') {
        ...
        if (closestDistance <= unit.range && unit.canAttack && unit.remainingActions > 0) {
            ...
            while (unit.canAttack && attackCount < unit.maxAttacks && closestPlayer.hp > 0 && unit.remainingActions > 0) {
                if (window.performAttack) await window.performAttack(unit, closestPlayer);
                attackCount++;
        ...
        if (unit.canMove && unit.remainingActions > 0) {
            if (window.moveTowardTarget) await window.moveTowardTarget(unit, closestPlayer.x, closestPlayer.y);
            ...
            if (newDistance <= unit.range && unit.canAttack && unit.remainingActions > 0) {
                if (window.logMessage) window.logMessage(`${unit.name} reaches the enemy!`, 'system');
                await delay(200);
        ...
        for (const player of players) {
            ...
            if (distance <= unit.range) {
                ...
                while (unit.canAttack && attackCount < unit.maxAttacks && player.hp > 0 && unit.remainingActions > 0) {
                    if (window.performAttack) await window.performAttack(unit, player);
                    attackCount++;
    ...
    for (let dx = -desiredDistance; dx <= desiredDistance; dx++) {
        for (let dy = -desiredDistance; dy <= desiredDistance; dy++) {
            ...
            if (checkX >= 0 && checkX < GRID_SIZE && 
                ...
                const players = window.gameState.units.filter(u => u.type === 'player' && u.hp > 0);
...
window.findPathToHealRange = findPathToHealRange;
