...
class Unit {
    constructor(type, name, x, y, forceName = null) {
        this.id = window.gameState.nextUnitId++;
        this.type = type;
    ...
    addInjury(injuryType) {
        ...
        if (window.logMessage) {
            window.logMessage(`${this.name} suffers a ${injury.name}!`, 'system');
        }
    }
    ...
    updateInjuries() {
        for (let i = this.injuries.length - 1; i >= 0; i--) {
            ...
            if (this.injuries[i].turnsRemaining <= 0) {
                if (window.logMessage) {
                    window.logMessage(`${this.name}'s ${this.injuries[i].name} has healed.`, 'system');
                }
    ...
    updateMorale(damageTaken, alliesLost = false) {
        ...
        if (this.morale < this.moraleThreshold) {
            ...
            this.attack -= penalty;
            if (window.logMessage) {
                window.logMessage(`${this.name} is shaken! (-${penalty} combat stats)`, 'system');
            }
        }
    ...
    flee() {
        ...
        this.morale = 0;
        if (window.logMessage) {
            window.logMessage(`${this.name} has broken and is fleeing!`, 'system');
        }
        if (window.soundSystem) {
            window.soundSystem.playFlee();
        }
        
        if (this.type === 'player') {
            window.gameState.battleStats.fleedUnits.push(this);
        }
        this.fleeTurns = 0;
    }
    ...
    performFlee() {
        ...
        if (Math.random() < rallyChance) {
            ...
            if (window.logMessage) {
                window.logMessage(`${this.name} rallies and rejoins the fight!`, 'system');
            }
            return false;
        }
        
        // Find closest enemy
        const enemies = window.gameState?.units?.filter(u => u.type !== this.type && !u.fleeing) || [];
        ...
        if (closestEnemy) {
            ...
            if (!getUnitAt(moveX, moveY) && window.gameState?.terrain[moveY][moveX] !== 'water') {
                this.x = moveX;
                this.y = moveY;
                if (window.logMessage) {
                    window.logMessage(`${this.name} flees in terror!`, 'system');
                }
                
                // Update display if this unit is selected
                if (window.gameState?.selectedUnit && window.gameState.selectedUnit.id === this.id) {
                    if (window.updateSelectedUnitStats) {
                        window.updateSelectedUnitStats();
                    }
                }
            }
    ...
    levelUp() {
        ...
        this.xp = Math.max(0, leftoverXp);  // Keep leftover, minimum 0
        
        this.xpToNext = Math.floor(this.xpToNext * 1.5);
        
        // REDUCED STAT GAINS (25-50% less than before)
        this.maxHp += 3;                    // Was +5
        ...
        if (this.classType === 'mage') {
            this.healPower += 2;            // Was +3
        }
        
        this.accuracy = Math.min(95, this.accuracy + 1);  // Was +2
        this.morale = Math.min(100, this.morale + 15);    // Was +20
    ...
    getAttackChance(target) {
        ...
        const targetTerrain = window.gameState?.terrain[target.y][target.x];
        ...
        const attackerTerrain = window.gameState?.terrain[this.y][this.x];
    ...
    checkHeroicRally() {
        ...
        if (this.hp > 0 && this.hp < this.maxHp * 0.2 && this.morale < 30) {
            if (Math.random() < 0.1) { // 10% chance
                ...
                this.fleeing = false;
                if (window.logMessage) {
                    window.logMessage(`${this.name} makes a heroic last stand! Morale restored!`, 'system');
                }
                return true;
            }
        }
